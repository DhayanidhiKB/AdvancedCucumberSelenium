...
Feature: "Create Billing Account/Rating Plan" request is "Submitted" to SAP for APPC New Customers belonging to Standard Pricing Points [Tier 4 to Tier 8] skipping proposal generation
  ...
  skipped by "Generate Manual Agreement Document" while adding pricing points for "APPC" product.
  "New Contract" is created and merged with manually uploaded one by managing relationships
  at required levels after performing credit assessment. Finally "Create Billing Account/Rating Plan"
  request is "Submitted" to SAP once the contract is activated.

  @OrganisationLevel
  Scenario Outline: As a '<user_type>' user, on-board new APPC customer['<org_name>'] belonging to standard pricing tier['<tier>'], price structure Z6, lodgement zones[Capital, Country, Metro] using "Generate Manual Agreement Document" and contract relationship applied at['<relationship>', '<level>']
    ...
    When '<user_type>' user performs credit assessment of the '<opp_name>' using abn['<abn>'] for "Charge Account" by entering '<industry>', '<email>', '<street_number>', '<sender>' and "Generate Manual Agreement Document" for '<product>' redirected to contract page
      | entity                        | type         | product         |
    ...
    @PTEST
    Examples:
      | user_type  | org_name                      | abn         | acn | b_street       | b_city    | b_state | b_code | p_street       | p_city    | p_state | p_code | title | first_name             | last_name          | email                         | opp_name                                                                                       | product                        | evaluated_spend | tier                             | transit_cover | transit_cover_type | sender                                    | industry | street_number | file_name                  | no_of_dsr |
      | Onboarding | PETER R CARTER PROVIDENT FUND | 68493895799 |     | 66 Hyde Street | Bellingen | NSW     | 2454   | 66 Hyde Street | Bellingen | NSW     | 2454   | Mr.   | Automation_Tester_1114 | Vinoth_Murali_1114 | vinoth.murali3@auspost.com.au | As onboarding generate manual agreement document for APPC new customer at org level_31_10_2023 | Australia Post Parcel Contract | 20000           | TIER 4 ($20,000.00 - $49,999.99) | 00            | Adhoc              | Automation_Tester_1114 Vinoth_Murali_1114 | B Mining | 66            | PDA-COMPASS-0000000084.pdf | 2         |
  ...
  Scenario Outline: As a '<user_type>' user, on-board new APPC customer['<org_name>'] belonging to standard pricing tier['<tier>'], price structure Z6, lodgement zones[Capital, Country, Metro] using "Generate Manual Agreement Document" and contract relationship applied at['<relationship>', '<level>']
    ...
    Examples:
      | user_type  | org_name                  | abn         | acn | b_street            | b_city    | b_state | b_code | p_street            | p_city    | p_state | p_code | title | first_name             | last_name          | email                         | opp_name                                                                                      | product                        | evaluated_spend | tier                               | transit_cover | transit_cover_type | sender                                    | industry | street_number | file_name                  | no_of_dsr |
      | Onboarding | FOUND SUPERANNUATION FUND | 19459519126 |     | 13 Marfayley Street | Salisbury | QLD     | 4107   | 13 Marfayley Street | Salisbury | QLD     | 4107   | Mr.   | Automation_Tester_1115 | Vinoth_Murali_1115 | vinoth.murali3@auspost.com.au | As onboarding generate manual agreement document for APPC new customer at ba level_31_10_2023 | Australia Post Parcel Contract | 350000          | TIER 7 ($250,000.00 - $499,999.99) | 70            | Always On          | Automation_Tester_1115 Vinoth_Murali_1115 | B Mining | 13            | PDA-COMPASS-0000000084.pdf | 2         |