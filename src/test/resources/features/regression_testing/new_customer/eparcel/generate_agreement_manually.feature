...
Feature: Create Billing Account/Rating Plan ID for EParcel New Customers with Standard Rate Card (Tier 4 to Tier 8) Pricing Points
  ...
  skipped by "Generate Manual Agreement Document" while adding pricing points for "EParcel" product.
  ...
  @EParcelBundle
  Scenario Outline: As OnBoarding user, on-board new customer['<org_name>'] using "Generate Manual Agreement Document" with EParcel Bundle belonging to category['<category>'] and price structure['<price_structure>']
    ...
    And '<opp_name>' cart saved with "eParcel" after validating '<category>', '<price_structure>', '<lodgement_code>', '<transit_cover>' and adding lodgement point '<lodgement_point>' by "Onboarding" user
    And '<opp_name>' cart saved with "eParcel Express" after validating '<category>', '<price_structure>', '<lodgement_code>', '<transit_cover>' and adding lodgement point '<lodgement_point>' by "Onboarding" user
    ...
    When "Onboarding" user performs credit assessment of the '<opp_name>' using abn['<abn>'] for "Charge Account" by entering '<industry>', '<email>', '<street_number>', '<sender>' and "Generate Manual Agreement Document" for "eParcel" redirected to contract page
      | entity                        | type         | product           |
    ...
    @STEST
    Examples:
      | org_name                     | abn         | acn       | b_street           | b_city    | b_state | b_code | p_street           | p_city    | p_state | p_code | title | first_name             | last_name          | email                         | opp_name                                                                             | category            | price_structure | lodgement_code | transit_cover | lodgement_point | sender                                    | industry | street_number | file_name                  |
      | NATION AUSTRALIA PTY LIMITED | 16100302850 | 100302850 | 18 Cocupara Avenue | Lindfield | NSW     | 2070   | 18 Cocupara Avenue | Lindfield | NSW     | 2070   | Mr.   | Automation_Tester_1013 | Vinoth_Murali_1013 | vinoth.murali3@auspost.com.au | As Onboarding generate manual agreement document for EParcel new customer_03_10_2023 | CAT2 - 2000 to 4999 | BANDED {Z6}     | 3000           | 10            | 3000            | Automation_Tester_1013 Vinoth_Murali_1013 | B Mining | 18            | PDA-COMPASS-0000000084.pdf |