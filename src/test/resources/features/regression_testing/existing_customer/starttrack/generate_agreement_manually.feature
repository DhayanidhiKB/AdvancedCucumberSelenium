...
Feature: "Create Rating Plan" request is "Submitted" to SAP for EParcel Existing Customers with Standard Rate Card (Tier 4 to Tier 8) Pricing Points
  ...
  for "StarTrack" product. "New Contract" is created by managing relationships at required levels once
  the proposal is accepted.Finally "Create Rating Plan" request is "Submitted" to SAP once the
  contract is activated.

  @STE
  Scenario Outline: As a Onboarding user, on-board existing StarTrack customer['<org_name>', '<abn>'] using "Generate Manual Agreement Document" belonging to tier['<tier>'] with service type['<service_type>'] and monthly spend['<monthly_spend>']
    ...
    @PTEST
    Examples:
      | user_type  | org_name                     | first_name             | last_name          | opp_name                                                                                                 | post_code      | monthly_spend | sender                                    | product       | service_type | primary_suburb | suburb         | cart_tier | file_name                  | no_of_dsr |
      | Onboarding | MCKENZIE SUPERANNUATION FUND | Automation_Tester_1021 | Vinoth_Murali_1021 | As Onboarding generate manual agreement document for StarTrack Road Express existing customer_31_10_2023 | 3000-MELBOURNE | 11000         | Automation_Tester_1021 Vinoth_Murali_1021 | Premium - STE | PRM          | 3000-MELBOURNE | 3000-MELBOURNE | Tier 4    | PDA-COMPASS-0000000084.pdf | 2         |