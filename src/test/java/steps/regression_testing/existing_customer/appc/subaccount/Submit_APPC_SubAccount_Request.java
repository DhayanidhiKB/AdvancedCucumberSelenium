...
public class Submit_APPC_SubAccount_Request {
    private final Base lBase;

    public Submit_APPC_SubAccount_Request(@NotNull Base base) {
        this.lBase = base;
    }

    @When("User submits sub-account request for an existing {string} level APPC contract by populating name[{string}, {string}], contract rates {string}, parcel send [{string}, {string}], {string} product , {string} eLMS, address[{string}, {string}, {string}, {string}, {string}] and lodgement point[{string}]")
