...
public class Submit_APPC_EParcel_SubAccount_Request {
    private final Base lBase;

    public Submit_APPC_EParcel_SubAccount_Request(@NotNull Base base) {
        this.lBase = base;
    }

    @When("User submits sub-account request for an existing {string} level APPC contract by populating name[{string}, {string}], contract rates {string} , parcel send [{string}, {string}], products[{string} and {string}], {string} eLMS, address[{string}, {string}, {string}, {string}, {string}] and lodgement point[{string}]")
    public void submit_sub_account_request(String contract_relationship, String sub_account_name, String sub_account_name2,
                                           String add_contract_rates, String parcel_send_login, String contact, String product1, String product2, String eLMS,
        ...
        this.lBase.salesforce.getLoginPage().getHeader().getAppNavigator()
                .getOrganisations().getOrganisationDetails()
                .related_billing_accounts()
                ...
                .choose(product1).choose(product2)
